...
import org.thingsboard.server.dao.nvr.NvrService;
...
public abstract class BaseController {
    ...
    @Autowired
    protected NvrService nvrService;
        ...
        try {
            ...
            switch (entityId.getEntityType()) {
                case DEVICE:
                    ...
                    return;
                case CAMERA:
                    checkCamera(cameraService.findCameraById(new CameraId(entityId.getId())));
                    return;
                case NVR:
                    checkNvr(nvrService.findNvrById(new NvrId(entityId.getId())));
                    return;
                case CUSTOMER:
    ...
    Camera checkCameraId(CameraId cameraId) throws ThingsboardException {
        try {
            validateId(cameraId, "Incorrect cameraId " + cameraId);
    ...
    Nvr checkNvrId(NvrId nvrId) throws ThingsboardException {
        try {
            validateId(nvrId, "Incorrect cameraId " + nvrId);
            Nvr nvr = nvrService.findNvrById(nvrId);
            checkNvr(nvr);
            return nvr;
        } catch (Exception e) {
            throw handleException(e, false);
    ...
    protected void checkCamera(Camera camera) throws ThingsboardException {
        checkNotNull(camera);
        checkTenantId(camera.getTenantId());
    ...
    protected void checkNvr(Nvr nvr) throws ThingsboardException {
        checkNotNull(nvr);
        checkTenantId(nvr.getTenantId());
    ...
    protected EntityView checkEntityViewId(EntityViewId entityViewId) throws org.thingsboard.server.common.data.exception.ThingsboardException {
        try {
    ...
    protected void checkEntityView(EntityView entityView) throws org.thingsboard.server.common.data.exception.ThingsboardException {
