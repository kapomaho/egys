...
import org.thingsboard.server.exception.InvalidCameraNvrRelationException;
...
public class EntityRelationController extends BaseController {
    ...
    public void saveRelation(@RequestBody EntityRelation relation) throws ThingsboardException {
        try {
            ...
            // Check relation between camera and nvr
            if (relation.getFrom().getEntityType() == EntityType.CAMERA && relation.getTo().getEntityType() == EntityType.NVR) {
                throw new InvalidCameraNvrRelationException("Invalid relation from Camera to Nvr!", ThingsboardErrorCode.INVALID_RELATION);
            } else if (relation.getFrom().getEntityType() == EntityType.NVR && relation.getTo().getEntityType() == EntityType.CAMERA
                    && !relation.getType().equals("Manages")){
                throw new InvalidCameraNvrRelationException("Relation between Nvr and Camera should be `Manages`", ThingsboardErrorCode.INVALID_RELATION);
            } else if(checkRelationExist(relation)) {
                throw new InvalidCameraNvrRelationException("Relation already exists!", ThingsboardErrorCode.INVALID_RELATION);
            } else if(relation.getFrom().getEntityType() == EntityType.NVR && relation.getTo().getEntityType() == EntityType.CAMERA &&
                    checkEntityHasToRelation(relation)) {
                throw new InvalidCameraNvrRelationException("Camera is already assigned to an Nvr!", ThingsboardErrorCode.INVALID_RELATION);
            }
    ...
    private boolean checkRelationExist(EntityRelation relation){
        EntityRelation r = null;
        try{
            r = relationService.getRelation(relation.getFrom(), relation.getTo(), relation.getType(), relation.getTypeGroup());
        } catch (Exception e) {
            return false;
        }
        return r != null;
    ...
    private boolean checkEntityHasToRelation(EntityRelation relation) {
        List<EntityRelation> r = null;
        try {
            r = relationService.findByToAndType(relation.getTo(), relation.getType(), relation.getTypeGroup());
        } catch (Exception e ) {
            return false;
        }
        return r.size() > 0;
