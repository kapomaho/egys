/**
 * Copyright © 2016-2018 The Thingsboard Authors
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.thingsboard.server.dao.nvr;

import com.datastax.driver.core.ResultSet;
import com.datastax.driver.core.ResultSetFuture;
import com.datastax.driver.core.Statement;
import com.datastax.driver.core.querybuilder.Select;
import com.datastax.driver.mapping.Result;
import com.google.common.base.Function;
import com.google.common.util.concurrent.Futures;
import com.google.common.util.concurrent.ListenableFuture;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;
import org.thingsboard.server.common.data.EntitySubtype;
import org.thingsboard.server.common.data.EntityType;
import org.thingsboard.server.common.data.Nvr;
import org.thingsboard.server.common.data.id.CameraId;
import org.thingsboard.server.common.data.id.CustomerId;
import org.thingsboard.server.common.data.page.TextPageLink;
import org.thingsboard.server.common.data.relation.EntityRelation;
import org.thingsboard.server.common.data.relation.RelationTypeGroup;
import org.thingsboard.server.dao.DaoUtil;
import org.thingsboard.server.dao.model.EntitySubtypeEntity;
import org.thingsboard.server.dao.model.nosql.NvrEntity;
import org.thingsboard.server.dao.nosql.CassandraAbstractSearchTextDao;
import org.thingsboard.server.dao.relation.RelationDao;
import org.thingsboard.server.dao.util.NoSqlDao;

import javax.annotation.Nullable;
import java.util.*;
import java.util.concurrent.ExecutionException;
import java.util.stream.Collectors;

import static com.datastax.driver.core.querybuilder.QueryBuilder.*;
import static org.thingsboard.server.dao.model.ModelConstants.*;

@Component
@Slf4j
@NoSqlDao
public class CassandraNvrDao extends CassandraAbstractSearchTextDao<NvrEntity, Nvr> implements NvrDao {

    @Autowired
    private RelationDao relationDao;

    @Override
    public Nvr save(Nvr domain) {
        Nvr nvr = super.save(domain);
        EntitySubtype entitySubtype = new EntitySubtype(nvr.getTenantId(), EntityType.NVR, nvr.getType());
        EntitySubtypeEntity entitySubtypeEntity = new EntitySubtypeEntity(entitySubtype);
        Statement statement = cluster.getMapper(EntitySubtypeEntity.class).saveQuery(entitySubtypeEntity);
        executeWrite(statement);
        return nvr;
    }

    @Override
    public Nvr findNvrByCameraId(UUID cameraId) {
        try {
            List<EntityRelation> cameraNvrRelations = relationDao.findAllByToAndType(new CameraId(cameraId), EntityRelation.MANAGES_TYPE, RelationTypeGroup.COMMON).get();
            UUID nvrId = null;
            for (EntityRelation er : cameraNvrRelations) {
                if (er.getFrom().getEntityType() == EntityType.NVR) {
                    nvrId = er.getFrom().getId();
                }
            }
            if (nvrId == null) {
                return null;
            }
            return findById(nvrId);
        } catch (InterruptedException | ExecutionException e) {
            log.error("Failed to fetch Nvr");
            throw new RuntimeException(e);
        }
    }


    @Override
    protected Class<NvrEntity> getColumnFamilyClass() {
        return NvrEntity.class;
    }

    @Override
    protected String getColumnFamilyName() {
        return NVR_COLUMN_FAMILY_NAME;
    }

    @Override
    public List<Nvr> findNvrsByTenantId(UUID tenantId, TextPageLink pagelink) {
        List<NvrEntity> nvrEntities = findPageWithTextSearch(
                NVR_BY_TENANT_AND_SEARCH_TEXT_COLUMN_FAMILY_NAME,
                Collections.singletonList(
                        eq(NVR_TENANT_ID_PROPERTY, tenantId)
                ),
                pagelink);
        return DaoUtil.convertDataList(nvrEntities);
    }

    @Override
    public List<Nvr> findNvrsByTenantIdAndType(UUID tenantId, String type, TextPageLink pageLink) {
        List<NvrEntity> nvrEntities = findPageWithTextSearch(
                NVR_BY_TENANT_BY_TYPE_AND_SEARCH_TEXT_COLUMN_FAMILY_NAME,
                Arrays.asList(
                        eq(NVR_TYPE_PROPERTY, type),
                        eq(NVR_TENANT_ID_PROPERTY, tenantId)
                ),
                pageLink);
        return DaoUtil.convertDataList(nvrEntities);
    }

    @Override
    public ListenableFuture<List<Nvr>> findNvrsByTenantIdAndIdsAsync(UUID tenantId, List<UUID> nvrIds) {
        Select select = select().from(getColumnFamilyName());
        Select.Where query = select.where();
        query.and(eq(NVR_TENANT_ID_PROPERTY, tenantId));
        query.and(in(ID_PROPERTY, nvrIds));
        return findListByStatementAsync(query);
    }

    @Override
    public List<Nvr> findNvrsByTenantIdAndCustomerId(UUID tenantId, UUID customerId, TextPageLink pageLink) {
        List<EntityRelation> customerNvrRelationList = relationDao.findRelations(new CustomerId(customerId),
                EntityRelation.CONTAINS_TYPE, RelationTypeGroup.NVR, EntityType.NVR);

        List<Nvr> nvrList = findNvrsByTenantId(tenantId, pageLink);

        return DaoUtil.filterDataListByRelation(customerNvrRelationList, nvrList);
    }

    @Override
    public List<Nvr> findNvrsByTenantIdAndCustomerIdAndType(UUID tenantId, UUID customerId, String type, TextPageLink pageLink) {
        List<EntityRelation> customerNvrRelationList = relationDao.findRelations(new CustomerId(customerId),
                EntityRelation.CONTAINS_TYPE, RelationTypeGroup.NVR, EntityType.NVR);

        List<Nvr> nvrList = findNvrsByTenantIdAndType(tenantId, type, pageLink);

        return DaoUtil.filterDataListByRelation(customerNvrRelationList, nvrList);
    }

    @Override
    public ListenableFuture<List<Nvr>> findNvrsByTenantIdCustomerIdAndIdsAsync(UUID tenantId, UUID customerId, List<UUID> nvrIds) {
        List<EntityRelation> customerNvrRelationList = relationDao.findRelations(new CustomerId(customerId),
                EntityRelation.CONTAINS_TYPE, RelationTypeGroup.NVR, EntityType.NVR);

        List<UUID> ids = customerNvrRelationList.stream().map(entityRelation -> entityRelation.getTo().getId()).collect(Collectors.toList());
        nvrIds.retainAll(ids);
        return findNvrsByTenantIdAndIdsAsync(tenantId, nvrIds);
    }

    @Override
    public Optional<Nvr> findNvrByTenantIdAndName(UUID tenantId, String name) {
        Select select = select().from(NVR_BY_TENANT_AND_NAME_VIEW_NAME);
        Select.Where query = select.where();
        query.and(eq(NVR_TENANT_ID_PROPERTY, tenantId));
        query.and(eq(NVR_NAME_PROPERTY, name));
        return Optional.ofNullable(DaoUtil.getData(findOneByStatement(query)));
    }

    @Override
    public Optional<Nvr> findNvrByTenantIdAndHost(UUID tenantId, String host) {
        Select select = select().from(NVR_BY_TENANT_AND_HOST_VIEW_NAME);
        Select.Where query = select.where();
        query.and(eq(NVR_TENANT_ID_PROPERTY, tenantId));
        query.and(eq(NVR_HOST_PROPERTY, host));
        return Optional.ofNullable(DaoUtil.getData(findOneByStatement(query)));
    }

    @Override
    public ListenableFuture<List<EntitySubtype>> findTenantNvrTypesAsync(UUID tenantId) {
        Select select = select().from(ENTITY_SUBTYPE_COLUMN_FAMILY_NAME);
        Select.Where query = select.where();
        query.and(eq(ENTITY_SUBTYPE_TENANT_ID_PROPERTY, tenantId));
        query.and(eq(ENTITY_SUBTYPE_ENTITY_TYPE_PROPERTY, EntityType.NVR));
        query.setConsistencyLevel(cluster.getDefaultReadConsistencyLevel());
        ResultSetFuture resultSetFuture = executeAsyncRead(query);
        return Futures.transform(resultSetFuture,
                new Function<ResultSet, List<EntitySubtype>>() {
                    @Nullable
                    @Override
                    public List<EntitySubtype> apply(@Nullable ResultSet resultSet) {
                        Result<EntitySubtypeEntity> result = cluster.getMapper(EntitySubtypeEntity.class).map(resultSet);
                        if (result != null) {
                            List<EntitySubtype> entitySubtypes = new ArrayList<>();
                            result.all().forEach((entitySubtypeEntity) ->
                                    entitySubtypes.add(entitySubtypeEntity.toEntitySubtype())
                            );
                            return entitySubtypes;
                        } else {
                            return Collections.emptyList();
                        }
                    }
                });
    }

}
