...
public interface AssetRepository extends CrudRepository<AssetEntity, String> {
    ...
    @Query("SELECT a FROM AssetEntity a WHERE a.tenantId = :tenantId " +
            "AND LOWER(a.searchText) LIKE LOWER(CONCAT(:textSearch, '%')) " +
            "AND a.id > :idOffset " +
            "AND a.id IN :assetIds ORDER BY a.id")
    List<AssetEntity> findByTenantIdAndSearchTextAndIdIn(@Param("tenantId") String tenantId,
    ...
    @Query("SELECT a FROM AssetEntity a WHERE a.tenantId = :tenantId " +
            ...
            "AND a.id > :idOffset " +
            "AND a.id IN :assetIds ORDER BY a.id")
    List<AssetEntity> findByTenantIdAndTypeAndSearchTextAndIdIn(@Param("tenantId") String tenantId,
                                                         @Param("type") String type,
                                                         @Param("assetIds") List<String> assetIds,
                                                         @Param("textSearch") String textSearch,
                                                         @Param("idOffset") String idOffset,
