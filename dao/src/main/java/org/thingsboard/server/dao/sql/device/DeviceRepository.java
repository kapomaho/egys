...
public interface DeviceRepository extends CrudRepository<DeviceEntity, String> {
    ...
    @Query("SELECT d FROM DeviceEntity d WHERE d.tenantId = :tenantId " +
            ...
            "AND d.id > :idOffset " +
            "AND d.id IN :deviceIds ORDER BY d.id")
    List<DeviceEntity> findByTenantIdAndSearchTextAndIdIn(@Param("tenantId") String tenantId,
    ...
    @Query("SELECT d FROM DeviceEntity d WHERE d.tenantId = :tenantId " +
            ...
            "AND d.id > :idOffset " +
            "AND d.id IN :deviceIds ORDER BY d.id")
    List<DeviceEntity> findByTenantIdAndTypeAndSearchTextAndIdIn(@Param("tenantId") String tenantId,
                                                                @Param("type") String type,
                                                                @Param("deviceIds") List<String> deviceIds,
                                                                @Param("textSearch") String textSearch,
                                                                @Param("idOffset") String idOffset,
