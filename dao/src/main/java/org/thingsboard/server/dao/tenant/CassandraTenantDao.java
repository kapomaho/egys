...
public class CassandraTenantDao extends CassandraAbstractSearchTextDao<TenantEntity, Tenant> implements TenantDao {
    ...
    public Tenant findByName(String name)
    {
        ...
        Select.Where query = select().from(getColumnFamilyName()).allowFiltering().where(eq(ModelConstants.SEARCH_TEXT_PROPERTY,name));
        log.trace("Execute query {}",query);
        TenantEntity entity = findOneByStatement(query);
