...
public class UserServiceImpl extends AbstractEntityService implements UserService {
    ...
    public User saveUserWithPassword(User user, String password){
        log.trace("Execuiting saveUserWithPassword [{}]");
        userValidator.validate(user);
        userValidator.validateForLdap(user);
        User savedUser = userDao.save(user);
        if(user.getId() == null){
            UserCredentials userCredentials = new UserCredentials();
            userCredentials.setEnabled(false);
    ...
    private DataValidator<User> userValidator =
            new DataValidator<User>() {
                ...
                @Override
                public void validateForLdap(User user)
                {
                    if(findUserByEmail(user.getEmail()) != null)
                    {
                        throw new DataValidationException("User is dublicated");
                    }
                }
    };
