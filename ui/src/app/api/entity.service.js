...
function EntityService($http, $q, $filter, $translate, $log, userService, deviceService, assetService, tenantService,
                       ...
                       entityViewService, cameraService, nvrService, types, utils) {
    var service = {
        getEntity: getEntity,
    ...
    function getEntityPromise(entityType, entityId, config) {
        ...
        switch (entityType) {
            ...
            case types.entityType.entityView:
                ...
                break;
            case types.entityType.camera:
                promise = cameraService.getCamera(entityId, true, config);
                break;
            case types.entityType.nvr:
                promise = nvrService.getNvr(entityId, true, config);
                break;
    ...
    function getEntitiesPromise(entityType, entityIds, config) {
        ...
        switch (entityType) {
            ...
            case types.entityType.asset:
                ...
                break;
            case types.entityType.camera:
                promise = cameraService.getCameras(entityIds, config);
                break;
            case types.entityType.nvr:
                promise = nvrService.getNvrs(entityIds, config);
                break;
            case types.entityType.tenant:
    ...
    function getEntitiesByPageLinkPromise(entityType, pageLink, config, subType) {
        ...
        switch (entityType) {
            ...
            case types.entityType.camera:
                if (user.authority === 'CUSTOMER_USER') {
                    promise = cameraService.getCustomerCameras(customerId, pageLink, config, subType);
                } else {
                    promise = cameraService.getTenantCameras(pageLink, config, subType);
                }
                break;
            case types.entityType.nvr:
                if (user.authority === 'CUSTOMER_USER') {
                    promise = nvrService.getCustomerNvrs(customerId, pageLink, config, subType);
                } else {
                    promise = nvrService.getTenantNvrs(pageLink, config, subType);
                }
                break;
            case types.entityType.user:
    ...
    function resolveAliasFilter(filter, stateParams, maxItems, failOnEmpty) {
        ...
        switch (filter.type) {
            ...
            case types.aliasFilterType.deviceType.value:
                ...
                break;
            case types.aliasFilterType.cameraType.value:
                getEntitiesByNameFilter(types.entityType.camera, filter.cameraNameFilter, maxItems, null, filter.cameraType).then(
                    function success(entities) {
                        if (entities && entities.length || !failOnEmpty) {
                            result.entities = entitiesToEntitiesInfo(entities);
                            deferred.resolve(result);
                ...
                break;
            case types.aliasFilterType.nvrType.value:
                getEntitiesByNameFilter(types.entityType.nvr, filter.nvrNameFilter, maxItems, null, filter.nvrType).then(
                    function success(entities) {
                        if (entities && entities.length || !failOnEmpty) {
                            result.entities = entitiesToEntitiesInfo(entities);
                            deferred.resolve(result);
            ...
            case types.aliasFilterType.entityViewSearchQuery.value:
                ...
                if (rootEntityType && rootEntityId) {
                    ...
                    } else if (filter.type == types.aliasFilterType.cameraSearchQuery.value) {
                        searchQuery.cameraTypes = filter.cameraTypes;
                        findByQueryPromise = cameraService.findByQuery(searchQuery, false);
                    }
    ...
    function filterAliasByEntityTypes(entityAlias, entityTypes) {
        ...
        if (filterAliasFilterTypeByEntityTypes(filter.type, entityTypes)) {
            switch (filter.type) {
                ...
                case types.aliasFilterType.cameraType.value:
                    return entityTypes.indexOf(types.entityType.camera) > -1 ? true : false;
                case types.aliasFilterType.nvrType.value:
                    return entityTypes.indexOf(types.entityType.nvr) > -1 ? true : false;
                case types.aliasFilterType.relationsQuery.value:
                ...
                case types.aliasFilterType.deviceSearchQuery.value:
                    return entityTypes.indexOf(types.entityType.device)  > -1 ? true : false;
                case types.aliasFilterType.cameraSearchQuery.value:
                    return entityTypes.indexOf(types.entityType.camera)  > -1 ? true : false;
                case types.aliasFilterType.nvrSearchQuery.value:
                    return entityTypes.indexOf(types.entityType.nvr)  > -1 ? true : false;
            }
        }
        return false;
    ...
    function filterAliasFilterTypeByEntityType(aliasFilterType, entityType) {
        switch (aliasFilterType) {
            ...
            case types.aliasFilterType.cameraType.value:
                return entityType === types.entityType.camera;
            case types.aliasFilterType.nvrType.value:
                return entityType === types.entityType.nvr;
            case types.aliasFilterType.relationsQuery.value:
                return true;
            case types.aliasFilterType.assetSearchQuery.value:
                return entityType === types.entityType.asset;
    ...
    function prepareAllowedEntityTypesList(allowedEntityTypes, useAliasEntityTypes) {
        ...
        switch(authority) {
            ...
            case 'TENANT_ADMIN':
                ...
                entityTypes.camera = types.entityType.camera;
                entityTypes.nvr = types.entityType.nvr;
                entityTypes.tenant = types.entityType.tenant;
    ...
    function getRelatedEntities(rootEntityId, entityType, entitySubTypes, maxLevel, keys, typeTranslatePrefix, relationType, direction) {
        ...
        if (!entitySearchQuery) {
            ...
            } else if (entityType == types.entityType.camera) {
                findByQueryPromise = cameraService.findByQuery(entitySearchQuery, true, {ignoreLoading: true});
            } else if (entityType == types.entityType.nvr) {
                findByQueryPromise = nvrService.findByQuery(entitySearchQuery, true, {ignoreLoading: true});
            }
            findByQueryPromise.then(
                function success(entities) {
                    var entitiesTasks = [];
    ...
    function deleteEntityPromise(entityId) {
        ...
        } else if (entityId.entityType == types.entityType.camera) {
            return cameraService.deleteCamera(entityId.id);
        } else if (entityId.entityType == types.entityType.nvr) {
            return nvrService.deleteNvr(entityId.id);
        }
    }
    ...
    function saveEntityPromise(entity) {
        ...
        if (entityType == types.entityType.asset) {
            ...
            return cameraService.saveCamera(entity);
        } else if (entityType == types.entityType.nvr) {
            return nvrService.saveNvr(entity);
        }
    }
    ...
    function constructRelatedEntitiesSearchQuery(rootEntityId, entityType, entitySubTypes, maxLevel, relationType, direction) {
        ...
        if (entityType == types.entityType.asset) {
            ...
            searchQuery.cameraTypes = entitySubTypes;
        } else if (entityType == types.entityType.nvr) {
            searchQuery.nvrTypes = entitySubTypes;
        } else {
