...
function ExpandFullscreen($compile, $document, $timeout) {
    ...
    var linker = function (scope, element, attrs) {
        ...
        scope.$watch('collapsed', function (newCollapsed, prevCollapsed) {
            if (newCollapsed != prevCollapsed) {
                if (scope.collapsed) {
                    element.addClass('tb-collapsed');
                    element.parent().find(".tb-widget-content").addClass('tb-collapsed-hide');
                } else {
                    element.removeClass('tb-collapsed');
                    element.parent().find(".tb-widget-content").removeClass('tb-collapsed-hide');
                }
                if (scope.onCollapseChanged) {
                    scope.onCollapseChanged({collapsed: scope.collapsed});
                }
            }
        });
        ...
        }, function (newCollapsed) {
            scope.collapsed = newCollapsed;
        });
        ...
        if (attrs.collapseButtonId) {
            $timeout(function() {
                var collapseButton = $('#' + attrs.collapseButtonId, element)[0];
        ...
        function renderCollapseButton(collapseButton) {
            var html = '<md-tooltip md-direction="{{collapsed ? \'bottom\' : \'' + tooltipDirection + '\'}}">' +
                '{{(collapsed ? \'fullscreen.cancelCollapse\' : \'fullscreen.collapse\') | translate}}' +
                '</md-tooltip>' +
                '<ng-md-icon ' + (buttonSize ? 'size="'+ buttonSize +'" ' : '') + 'icon="{{collapsed ? \'expand_more\' : \'expand_less\'}}" ' +
                'options=\'{"easing": "circ-in-out", "duration": 375, "rotation": "none"}\'>' +
                '</ng-md-icon>';

            if (collapseButton) {
                collapseButton = angular.element(collapseButton);
                if (scope.hideCollapseButton() || scope.expanded) {
                    collapseButton.remove();
                } else {
                    collapseButton.attr('md-ink-ripple', 'false');
                    collapseButton.append(html);

                    $compile(collapseButton.contents())(scope);

                    collapseButton.on("click", scope.toggleCollapse);
                }
            } else if (!scope.hideCollapseButton() && !scope.expanded) {
                var button = angular.element('<md-button class="tb-fullscreen-button-style tb-fullscreen-button-pos md-icon-button" ' +
                    'md-ink-ripple="false" ng-click="toggleCollapse($event)">' +
                    html +
                    '</md-button>');
