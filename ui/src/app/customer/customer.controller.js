...
export default function CustomerController(customerService, $state, $stateParams, $translate, types) {
    ...
    var customerActionsList = [
        ...
        {
            onAction: function ($event, item) {
                openCustomerCameras($event, item);
            },
            name: function() { return $translate.instant('camera.cameras') },
            details: function(customer) {
                if (customer && customer.additionalInfo && customer.additionalInfo.isPublic) {
                    return $translate.instant('customer.manage-public-cameras')
                } else {
                    return $translate.instant('customer.manage-customer-cameras')
                }
            },
            icon: "camera_alt"
        },
        {
            onAction: function ($event, item) {
                openCustomerNvrs($event, item);
            },
            name: function() { return $translate.instant('nvr.nvrs') },
            details: function(customer) {
                if (customer && customer.additionalInfo && customer.additionalInfo.isPublic) {
                    return $translate.instant('customer.manage-public-nvrs')
                } else {
                    return $translate.instant('customer.manage-customer-nvrs')
                }
            },
            icon: "video_library"
        },
    ...
    function openCustomerCameras($event, customer) {
        if ($event) {
            $event.stopPropagation();
        }
        $state.go('home.customers.cameras', {customerId: customer.id.id});
    }

    function openCustomerNvrs($event, customer) {
        if ($event) {
            $event.stopPropagation();
        }
        $state.go('home.customers.nvrs', {customerId: customer.id.id});
    }
