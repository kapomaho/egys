            ...
            if (scope.filter && scope.filter.type) {
                ...
                switch (scope.filter.type) {
                    ...
                    case types.aliasFilterType.cameraType.value:
                        var cameraType = scope.filter.cameraType;
                        prefix = scope.filter.cameraNameFilter;
                        if (prefix && prefix.length) {
                            scope.filterDisplayValue = $translate.instant('alias.filter-type-camera-type-and-name-description', {cameraType: cameraType, prefix: prefix});
                        } else {
                            scope.filterDisplayValue = $translate.instant('alias.filter-type-camera-type-description', {cameraType: cameraType});
                        }
                        break;
                    case types.aliasFilterType.nvrType.value:
                        var nvrType = scope.filter.nvrType;
                        prefix = scope.filter.nvrNameFilter;
                        if (prefix && prefix.length) {
                            scope.filterDisplayValue = $translate.instant('alias.filter-type-nvr-type-and-name-description', {nvrType: nvrType, prefix: prefix});
                        } else {
                            scope.filterDisplayValue = $translate.instant('alias.filter-type-nvr-type-description', {nvrType: nvrType});
                        }
                        break;
                    ...
                    case types.aliasFilterType.entityViewSearchQuery.value:
                        ...
                        } else if (scope.filter.type == types.aliasFilterType.cameraSearchQuery.value) {
                            var cameraTypesQuoted = [];
                            scope.filter.cameraTypes.forEach(function(cameraType) {
                                cameraTypesQuoted.push("'"+cameraType+"'");
                            });
                            var cameraTypesText = cameraTypesQuoted.join(', ');
                            translationValues.cameraTypes = cameraTypesText;
                            scope.filterDisplayValue = $translate.instant('alias.filter-type-camera-search-query-description',
                                translationValues
                            );
                        } else if (scope.filter.type == types.aliasFilterType.nvrSearchQuery.value) {
                            var nvrTypesQuoted = [];
                            scope.filter.nvrTypes.forEach(function(nvrType) {
                                nvrTypesQuoted.push("'"+nvrType+"'");
                            });
                            var nvrTypesText = nvrTypesQuoted.join(', ');
                            translationValues.nvrTypes = nvrTypesText;
                            scope.filterDisplayValue = $translate.instant('alias.filter-type-nvr-search-query-description',
                                translationValues
                            );
