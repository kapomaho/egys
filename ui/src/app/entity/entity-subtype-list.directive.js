...
/*@ngInject*/
export default function EntitySubtypeListDirective($compile, $templateCache, $q, $mdUtil, $translate, $filter, types, assetService, deviceService, cameraService, nvrService, entityViewService) {
    ...
    var linker = function (scope, element, attrs, ngModelCtrl) {
        ...
        } else if (scope.entityType == types.entityType.camera) {
            scope.placeholder = scope.tbRequired ? $translate.instant('camera.enter-camera-type')
                : $translate.instant('camera.any-camera');
            scope.secondaryPlaceholder = '+' + $translate.instant('camera.camera-type');
            scope.noSubtypesMathingText = 'camera.no-camera-types-matching';
            scope.subtypeListEmptyText = 'camera.camera-type-list-empty';
        } else if (scope.entityType == types.entityType.nvr) {
            scope.placeholder = scope.tbRequired ? $translate.instant('nvr.enter-nvr-type')
                : $translate.instant('nvr.any-nvr');
            scope.secondaryPlaceholder = '+' + $translate.instant('nvr.nvr-type');
            scope.noSubtypesMathingText = 'nvr.no-nvr-types-matching';
            scope.subtypeListEmptyText = 'nvr.nvr-type-list-empty';
        ...
        function loadSubTypes() {
            ...
            if (!scope.entitySubtypes) {
                ...
                if (scope.entityType == types.entityType.asset) {
                    ...
                    entitySubtypesPromise = cameraService.getCameraTypes();
                } else if (scope.entityType == types.entityType.nvr) {
                    entitySubtypesPromise = nvrService.getNvrTypes();
                } else if (scope.entityType == types.entityType.entityView) {
