...
import TbOpenStreetGeoserverMap from './openstreet-geoserver-map';
import TbImageMap from './image-map';
import TbTencentMap from './tencent-map';

import {arraysEqual, fillPattern, fillPatternWithActions, processPattern, toLabelValueMap} from './widget-utils';
...
export default class TbMapWidgetV2 {
    constructor(mapProvider, drawRoutes, ctx, useDynamicLocations, $element) {
        ...
        } else if (mapProvider === 'openstreet-geoserver-map') {
            this.map = new TbOpenStreetGeoserverMap($element, this.utils, initCallback, settings.mapServerIP, settings.mapLayers, this.defaultZoomLevel, this.dontFitMapBounds, minZoomLevel);
        }
    }
