...
export default class TbOpenStreetGeoserverMap {

    constructor($containerElement, utils, initCallback, mapServerIP, mapLayers, defaultZoomLevel, dontFitMapBounds, minZoomLevel) {
        ...
        this.host = 'http://localhost:8099/geoserver/Turkiye/wms/';
        if (mapServerIP != null) {
            if (mapServerIP.startsWith("http"))
                this.host = mapServerIP + "/geoserver/Turkiye/wms/";
            else
                this.host = "http://" + mapServerIP + "/geoserver/Turkiye/wms/";
        }

        this.mapLayers = (mapLayers == null ? 'Turkiye:Turkiye' : mapLayers);

        var tileLayer = L.tileLayer.wms(this.host, {
